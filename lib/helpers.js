...
function createGitignore(applicationFolder) {
  return fs.writeFile(`${applicationFolder}/.gitignore`, `node_modules
.env
dist`);
}

async function copyFiles(src, applicationFolder, config) {
  try {
    console.log(chalk.green('Setting up files'), applicationFolder);
    if(applicationFolder !== '.') await fs.mkdir(applicationFolder);
    const files = await fs.readdir(src);
    
    const stats = await Promise.all(files.filter(file => file !== '.gitKeep').map(file => fs.lstat(path.join(src, file))));
    
    await Promise.all(stats.map((stat, i) => {
      const srcFilePath = path.join(src, files[i]);
      const destFilePath = path.join(applicationFolder, files[i]);
      
      if(stat.isDirectory()) return copyFiles(srcFilePath, destFilePath, config);
...
async function makeTemplate(src, dest, config) {
  const content = await renderFile(src, config);
  return fs.writeFile(dest, content);
}

async function renderFile(src, config) {
  const content = await fs.readFile(src, { encoding: 'utf8' });
  return render(content, config);
}

function installDependencies(applicationFolder, devDependencies, dependencies) {
